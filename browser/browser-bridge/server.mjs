...
const CHROMIUM_CMD = '/usr/local/bin/chromium';
...
async function discoverProfileDir(helperSid) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (val) {
      profileDir = val;
      console.log('[bridge] discovered profile dir:', profileDir);
      return;
    }
  }
}
  ...
  browserConnecting = (async () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        ...
        console.log('[bridge] browser-level CDP connected');
      ...
      ws.on('close', () => {
        console.log('[bridge] browser-level CDP disconnected');
        browserWs = null;
        browserConnecting = null;
      ...
      ws.on('error', (err) => {
        console.error('[bridge] browser CDP error:', err.message);
        browserConnecting = null;
        reject(err);
      });
    });
...
server.on('upgrade', (req, socket, head) => {
  console.log('[bridge] WS upgrade request:', req.url);
  if (req.url.match(/\/ws(\?|$)/)) {
    ...
    console.log('[bridge] WS upgrade rejected — no matching path:', req.url);
    socket.destroy();
  }
});
...
devtoolsWss.on('connection', async (client, req) => {
  console.log('[bridge] devtools client connected, url:', req.url);

  const url = new URL(req.url, 'http://localhost');
  const targetId = url.searchParams.get('target');
  console.log('[bridge] devtools requested target:', targetId);
  ...
  client.on('close', () => {
    console.log('[bridge] devtools client disconnected');
  ...
  try {
    ...
    console.log('[bridge] devtools session attached:', sessionId, 'target:', target.targetId);
  } catch (err) {
    console.error('[bridge] devtools attach failed:', err.message);
  ...
  console.log('[bridge] devtools proxy ready, replayed', bufferedCount, 'buffered messages');
});
...
viewerWss.on('connection', async (client, req) => {
  ...
  console.log('[bridge] viewer client connected', preferredTarget ? `(preferred: ${preferredTarget})` : '');
  ...
  async function reconcileTabs() {
    ...
    try {
      ...
      if (currentTargetId && !currentIds.has(currentTargetId)) {
        ...
        console.log('[bridge] reconcile: active tab %s gone, next=%s (old order had %d tabs, now %d)',
      ...
      console.error('[bridge] reconcileTabs error:', err.message);
    } finally {
      reconciling = false;
    }
  }
  ...
  async function connectToTarget(targetId) {
    ...
    if (sessionId) {
      ...
      sessionId = null;
      zoomScriptId = null;
      console.log('[bridge] connectToTarget: detach old %dms', Date.now() - ct0);
    }
    ...
    let ct1 = Date.now();
    await ensureBrowserConnection();
    console.log('[bridge] connectToTarget: ensureBrowser %dms', Date.now() - ct1);
    if (!downloadBehaviorSet) {
      ...
      console.log('[bridge] download behavior set:', dlPath);
    }
    ct1 = Date.now();
    const target = await findPageTarget(targetId);
    currentTargetId = target.targetId;
    console.log('[bridge] connectToTarget: findPageTarget %dms', Date.now() - ct1);
    ct1 = Date.now();
    sessionId = await attachToTarget(currentTargetId);
    console.log('[bridge] connectToTarget: attach %dms (session=%s target=%s)', Date.now() - ct1, sessionId, currentTargetId);
    ...
    sessionHandlers.set(sessionId, (msg) => {
      ...
      if (msg.method === 'Inspector.detached') {
        console.log('[bridge] viewer session detached:', msg.params?.reason);
        screencastStarted = false;
    ...
    if (activateResp.error) console.error('[bridge] activateTarget failed:', activateResp.error.message);
    console.log('[bridge] connectToTarget: activateTarget %dms', Date.now() - t1);
    t1 = Date.now();
    await sessionCommand(sessionId, 'Page.bringToFront');
    console.log('[bridge] connectToTarget: bringToFront %dms', Date.now() - t1);
    t1 = Date.now();
    await sessionCommand(sessionId, 'Page.enable');
    console.log('[bridge] connectToTarget: Page.enable %dms', Date.now() - t1);
    t1 = Date.now();
    ...
    console.log('[bridge] connectToTarget: getFrameTree %dms', Date.now() - t1);
    t1 = Date.now();
    ...
    if (scResp.error) console.error('[bridge] startScreencast failed:', scResp.error.message);
    screencastStarted = true;
    console.log('[bridge] connectToTarget: startScreencast %dms', Date.now() - t1);
    console.log('[bridge] connectToTarget: total %dms', Date.now() - ct0);
  ...
  function onTargetDestroyed(destroyedId) {
    ...
    console.log('[bridge] current target destroyed externally:', destroyedId);
    reconcileTabs();
  }
  ...
  client.on('close', () => {
    ...
    console.log('[bridge] viewer client disconnected');
    switching = true;
    if (sessionId) {
  ...
  (async function connectLoop() {
    ...
    while (!isClosed) {
      try {
        ...
        if (attempts === 0) {
          console.error('[bridge] initial connect failed, will retry:', err.message);
        }
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case 'closeTab':
          if (msg.targetId) {
            if (!knownTabs.has(msg.targetId)) {
              console.log('[bridge] closeTab: %s not in knownTabs, ignoring', msg.targetId);
              break;
            }
            const t0 = Date.now();
        ...
        case 'browserRestart':
          ...
          try {
            await execFileAsync(CHROMIUM_CMD, ['chrome', 'stop'], { timeout: 15000 }).catch(() => {});
            await execFileAsync(CHROMIUM_CMD, ['chrome', 'start'], { timeout: 15000 });
        ...
        case 'browserStop':
          ...
          if (browserWs) { browserWs.close(); browserWs = null; }
          try {
            await execFileAsync(CHROMIUM_CMD, ['chrome', 'stop'], { timeout: 15000 });
        ...
        case 'bridgeRestart': {
          ...
          setTimeout(() => {
            const child = execFile(CHROMIUM_CMD, ['bridge', 'restart'], {
      ...
      console.error('[bridge] viewer message error:', err?.message || err);
    }
  });
});

server.listen(PORT, '0.0.0.0', () => {
  console.log(`[bridge] Browser bridge listening on http://0.0.0.0:${PORT}`);
});
